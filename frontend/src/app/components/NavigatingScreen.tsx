  ...
  return (
    <div className="w-full h-full relative overflow-hidden bg-[#EBF5FF]">
      ...
      <div className="absolute top-4 left-4 right-4 z-10 bg-white rounded-[24px] shadow-lg p-6 flex items-center justify-between">
        <div>
          <h3 className="text-[28px] font-black text-gray-800">{route.name}</h3>
          <div className="flex items-center gap-4 text-gray-500 font-bold mt-1 text-[20px]">
      ...
      {showConfirm && (
        <div className="absolute inset-0 bg-black/40 z-20 flex flex-col justify-end animate-[fadeIn_0.2s_ease-out]">
          <div className="w-full bg-white rounded-t-[36px] p-8 pb-12 flex flex-col items-center animate-[slideUp_0.3s_ease-out]">
            ...
            <h2 className="text-[30px] font-black text-gray-800 mb-5">처음으로 돌아갈까요?</h2>
            <p className="text-gray-500 font-bold text-[22px] text-center mb-12 leading-relaxed break-keep">
              안내를 멈추고<br/>처음 화면으로 돌아갑니다.
            </p>
            <div className="flex gap-4 w-full">
              <button onClick={() => setShowConfirm(false)} className="flex-1 bg-[#F3F4F6] text-gray-700 py-6 rounded-2xl font-black text-[26px] active:scale-95 transition-transform">
                아니오
              </button>
              <button onClick={onRestart} className="flex-1 bg-[#3B82F6] text-white py-6 rounded-2xl font-black text-[26px] shadow-md active:scale-95 transition-transform">
                네
              </button>
            </div>
          </div>
